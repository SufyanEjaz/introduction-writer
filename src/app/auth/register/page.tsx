...
const Register = () => {
  ...
  const [errors, setErrors] = useState<{ email?: string; password?: string; confirm_password?: string; name?: string; apiError?: string }>({});
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const customError = 'An error occurred while registration. Please use valid data.';
    ...
    try {
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        setErrors({
          ...
          confirm_password: apiErrors.confirm_password ? apiErrors.confirm_password[0] : '',
  ...
  const handleInputChange = (field: 'name' | 'email' | 'password' | 'confirm_password', value: string) => {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center">
      <form onSubmit={handleRegister} className="bg-white p-6 rounded shadow-md w-96">
        ...
        <div className="mb-4">
          ...
          <input
            type="password"
            className={`w-full border rounded p-2 ${errors.confirm_password ? 'border-red-500' : 'border-gray-300'}`}
        ...
        {/* API Error Message */}
        {!(errors.email || errors.password || errors.confirm_password) && errors.apiError && (
